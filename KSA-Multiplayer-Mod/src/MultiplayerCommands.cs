...
namespace KSA.Mods.Multiplayer
{
    public static class MultiplayerCommands
    {
        public static void RegisterCommands()
        {
            ...
            terminal.RegisterCommand(new Action<string, int, int>(HostGame));
        ...
        [TerminalAction("mp_host", "Host a multiplayer session - usage: mp_host <playerName> <port> <maxPlayers>", ArgParseMode.Default)]
        public static void HostGame(string playerName, int port, int maxPlayers)
        {
            var manager = ModEntry.GetMultiplayerManager();
            if (manager == null) return;
            
            if (manager.HostSession(playerName, (ushort)port, (ushort)maxPlayers))
                DefaultCategory.Log.Info("Hosting multiplayer session", "HostGame", nameof(MultiplayerCommands));
        }
