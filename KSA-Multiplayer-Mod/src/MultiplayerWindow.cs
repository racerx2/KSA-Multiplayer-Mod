...
namespace KSA.Mods.Multiplayer
{
    public class MultiplayerWindow : ImGuiWindow, IStaticWindow
    {
        ...
        private void DrawConnectionSection()
        {
            ...
            ImGui.InputText("##port", _portInput);
            ...
            if (!_multiplayerManager.IsConnected)
            {
                if (ImGui.Button("Connect") && ushort.TryParse(_portInput.ToString(), out ushort portNum))
                    _ = _multiplayerManager.JoinSession(_playerNameInput.ToString(), _serverIpInput.ToString(), portNum);
                
                ImGui.SameLine();
                if (ImGui.Button("Host Server") && ushort.TryParse(_portInput.ToString(), out ushort p))
                    _multiplayerManager.HostSession(_playerNameInput.ToString(), p, 8);
            }
            else
            {
